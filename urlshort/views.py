...
def createShortURL(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if ShortURL.objects.filter(original_url=original_website).exists():
                return HttpResponse('URL already exist')
            ...
            while len(ShortURL.objects.filter(short_url=random_chars)) != 0:
                for i in range(6):
                    random_chars += random.choice(random_chars_list)
            d = timezone.now()
            s = ShortURL(original_url=original_website, short_url=random_chars, time_data_created=d)
            s.save()
    ...
    return HttpResponse("Enter URL in correct format")
